...
public:
    ...
    //minwoo: Use variables in imuPreintegration.cpp
    ...
    void addOdomFactor()
    {
        if (cloudKeyPoses3D->points.empty())
        {
            ...
            gtsam::PriorFactor<gtsam::Pose3> pose_factor(X(cloudKeyPoses3D->size()), poseTo, isDegenerate ? correctionNoise2 : correctionNoise);
    ...
    void addImuFactor()
    {
        ...
        {  
            ...
            {
                if (ROS_TIME(&imuQueue.front()) < timeLaserInfoCur)
                {
                    lastImuT_opt = ROS_TIME(&imuQueue.front());
                    imuQueue.pop_front();
                }
                else
                    break;
            ...
            gtSAMgraph.add(priorVel);
            // initial bias
            prevBias_ = gtsam::imuBias::ConstantBias();
            gtsam::PriorFactor<gtsam::imuBias::ConstantBias> priorBias(B(0), prevBias_, priorBiasNoise);
            gtSAMgraph.add(priorBias);
        ...
        else
        {
            while (!imuQueue.empty())
            {
                ...
                sensor_msgs::Imu *thisImu = &imuQueue.front();
                double imuTime = ROS_TIME(thisImu);
                if (imuTime < timeLaserInfoCur)
                {
            ...
            gtsam::ImuFactor imu_factor(X(cloudKeyPoses3D->size()-1), V(cloudKeyPoses3D->size()-1), X(cloudKeyPoses3D->size()), V(cloudKeyPoses3D->size()), B(cloudKeyPoses3D->size()-1), preint_imu);
            gtSAMgraph.add(imu_factor);

            // add imu bias between factor
            gtSAMgraph.add(gtsam::BetweenFactor<gtsam::imuBias::ConstantBias>(B(cloudKeyPoses3D->size()-1), B(cloudKeyPoses3D->size()), gtsam::imuBias::ConstantBias(),
                            gtsam::noiseModel::Diagonal::Sigmas(sqrt(imuIntegratorOpt_->deltaTij()) * noiseModelBetweenBias)));     
            
            gtsam::NavState propState_ = imuIntegratorOpt_->predict(prevState_, prevBias_);
            initialEstimate.insert(V(cloudKeyPoses3D->size()), propState_.v());
            initialEstimate.insert(B(cloudKeyPoses3D->size()), prevBias_);                               
    ...
    {  
        ...
        latestEstimate = isamCurrentEstimate.at<Pose3>(X(cloudKeyPoses3D->size()));
        prevVel_   = isamCurrentEstimate.at<gtsam::Vector3>(V(cloudKeyPoses3D->size()));
        prevState_ = gtsam::NavState(latestEstimate, prevVel_);
        prevBias_  = isamCurrentEstimate.at<gtsam::imuBias::ConstantBias>(B(cloudKeyPoses3D->size()));
        imuIntegratorOpt_->resetIntegrationAndSetBias(prevBias_);
        ...
        //poseCovariance = isam->marginalCovariance(X(cloudKeyPoses3D->size()-1));
